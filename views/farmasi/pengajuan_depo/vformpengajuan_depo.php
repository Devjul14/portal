<script>
    $(document).ready(function(){
        ...
        $(".back").click(function(){
            window.location = "<?php echo site_url('permintaan/pengajuan_depo')?>";
            return false;
        });
        $(".hapus").click(function(){
            var kode= $(this).attr("href");
            window.location = "<?php echo site_url('permintaan/hapusitem_pengajuan')?>/"+kode;
            return false;
        });
        $("[name='nama']").typeahead({
        ...
        $('.dataChange2').click(function(evt) {
            ...
            dataInputField.focus().blur(function(){
                var inputval = dataInputField.val();
                $(this).remove();
                changeData2(inputval,no_pengajuan,kode);
            }).keyup(function(evt) {
                if (evt.keyCode == 13) {
                    var inputval = dataInputField.val();
                    $(this).remove();
    ...
    var changeData = function(value,no_pengajuan,kode){
        $.ajax({
            url: "<?php echo site_url('permintaan/changedata_pengajuan');?>", 
            type: 'POST', 
    ...
    var changeData2 = function(value,no_pengajuan,kode){
        $.ajax({
            url: "<?php echo site_url('permintaan/changedata_pengajuan2');?>", 
            type: 'POST', 
            data: {no_pengajuan: no_pengajuan,kode: kode, value: value}, 
            success: function(){
               location.reload();
...
<div class="col-md-12">
    ...
    <div class="box box-info">
        <?php echo form_open("permintaan/simpanpengajuan/".$action,array("id"=>"formsimpan","class"=>"form-horizontal"));?>
        <div class="box-body">
    ...
    <?php if ($q1): ?>
        <div class="box box-info">
            <?php echo form_open("permintaan/simpanitem_pengajuan",array("id"=>"formsimpan","class"=>"form-horizontal"));?>
                            ...
                            foreach ($q->result() as $value) {
                                echo "
                                    <tr>
                                        <td>".$value->nama."</td>
                                        <td class='text-center'>
                                            <a href='#' class='qty dataChange' kode='".$value->kode_obat."' id='".$value->no_pengajuan."'>".$value->jumlah."</a>
                                        </td>
                                        <td>".$value->pak1."</td>
                                        <td>".$value->pak2."</td>
                                        <td>".$value->isi."</td>
                                        <td>".$value->jumlah_kecil."</td>
                                        <td class='text-center'><button class='hapus btn btn-danger' href='".$value->no_pengajuan."/".$value->kode_obat."'><i class='fa fa-trash'></></button</td>
                                    </tr>
                                ";
                            }
                        ?>
