<script>
    $(document).ready(function(){
        ...
        $(".back").click(function(){
            window.location = "<?php echo site_url('permintaan/permintaan_obat')?>";
            return false;
        });
        $(".hapus").click(function(){
            var kode= $(this).attr("href");
            window.location = "<?php echo site_url('permintaan/hapusitemrk')?>/"+kode;
            return false;
        });
        $("select[name='no_renbut']").change(function(){
            var kode= $(this).val();
            window.location = "<?php echo site_url('permintaan/formpermintaan_obat')?>/"+kode;
            return false;
        });
        $("[name='nama']").typeahead({
            source: function(query, process) {
                ...
                map = {};
                var data = $.ajax({
                    url : "<?php echo base_url();?>rk/getobat",
                    method : "POST",
                    async: false,
                    data : {nama: query}
                }).responseText;
            ...
            updater: function(item) {
                var n = item.split(" | ");
                var no_renbut= $("[name='no_renbut']").val();
                var kode = n[0];
                $.ajax({
                    url : "<?php echo base_url();?>rk/simpanitem_rk",
                    method : "POST",
                    data : {kode: kode,no_renbut: no_renbut},
...
<div class="col-md-12">
    ...
    <div class="box box-info">
        <?php echo form_open("permintaan/simpanpermintaan/".$action,array("id"=>"formsimpan","class"=>"form-horizontal"));?>
        <div class="box-body">
            ...
            <div class="form-group">
                ...
                <div class="col-md-10">
                   <textarea class="form-control" name="keterangan" <?php echo $a ?> required><?php echo $keterangan ?></textarea>
                </div>
            </div>
        </div>
    </div>
                        ...
                        <?php
                            ...
                            foreach ($irk->result() as $item) {
                                if ($item->sisa_jumlah>0) {
                                    $jumlah = "<input type='number' class='form-control' name='jumlah_kebutuhan[".$item->kode_obat."]' max='".$item->sisa_jumlah."' value='".$item->sisa_jumlah."'>";
                                } else {
                                    $jumlah = "-";
                                }
                                ...
                                echo "
                                    <tr>
                                        ...
                                        <td>
                                            ".$jumlah."
                                            <input type='hidden' class='form-control' name='jumlah_rk[".$item->kode_obat."]' value='".$item->jumlah."'>
                                            <input type='hidden' class='form-control' name='harga[".$item->kode_obat."]' value='".$item->hps."'>
                                            <input type='hidden' class='form-control' name='sisa_jumlah[".$item->kode_obat."]' value='".$item->sisa_jumlah."'>
                                        </td>
                                    </tr>
                                ";
                                $sj += $item->sisa_jumlah;
            ...
            <div class="box box-info">
                <div class="box-body">
                    <table class="table table-striped table-bordered" id="data">
                        ...
                        <!-- <input type='hidden' class='form-control' name='sisa_jumlah[".$item->kode_obat."]' value='".$item->sisa_jumlah."'> -->
                        <!-- <input type='number' class='form-control' name='jumlah_kebutuhan[".$item->kode_obat."]' value='".$item->jumlah."' >
                        <input type='hidden' class='form-control' name='jumlah_rk[".$item->kode_obat."]' value='".$item->jumlah_rk."'> -->
                        <?php
                ...
                <div class="box-body">
                    <table class="table table-striped table-bordered" id="data">
                        ...
                        <?php
                            foreach ($irk->result() as $item) {
                                if ($item->jumlah>1) {
                                    $jumlah = "<input type='number' class='form-control' name='jumlah_kebutuhan[".$item->kode_obat."]' max='".$item->sisa_jumlah."'>";
                                } else {
                                    $jumlah = $item->jumlah;
                                }
