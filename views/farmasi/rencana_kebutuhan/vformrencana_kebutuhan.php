<script>
    $(document).ready(function(){
        ...
        $(".back").click(function(){
            window.location = "<?php echo site_url('rk/rencana_kebutuhan')?>";
            return false;
        });
        $(".hapus").click(function(){
            var kode= $(this).attr("href");
            window.location = "<?php echo site_url('rk/hapusitemrk')?>/"+kode;
            return false;
        });
        $("[name='periode']").change(function(){
            var kode= $(this).val();
            window.location = "<?php echo site_url('rk/formrencana_kebutuhan')?>/"+kode;
            return false;
        });
    });
</script>
...
<div class="col-md-12">
    ...
    <div class="box box-info">
        <?php echo form_open("rk/simpanrencana_kebutuhan/".$action,array("id"=>"formsimpan","class"=>"form-horizontal"));?>
        <div class="box-body">
                            ...
                            foreach ($p->result() as $value) {
                                ...
                                echo "
                                    <tr>
                                        ...
                                        <td>".number_format(($value->hps)*($value->qty),0,',','.')."</td>
                                        <td>
                                            <input type='number' class='form-control' name='jumlah_kebutuhan[".$value->kode_obat."]' min='".$value->qty."' max='".$value->qty."' value='".$value->qty."'>
                                            <input type='hidden' class='form-control' name='jumlah_pengajuan[".$value->kode_obat."]' value='".$value->sisa_jumlah."'>
                                            <input type='hidden' class='form-control' name='hps[".$value->kode_obat."]' value='".$value->hps."'>
                                            <input type='hidden' class='form-control' name='jumlah_kecil[".$value->kode_obat."]' value='".($value->qty*$value->isi)."'>
                                        </td>
                                    </tr>
                                ";
                            }
