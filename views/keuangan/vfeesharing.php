<script>
	...
	$(document).ready(function(e){
        ...
        $(".excel").click(function(){
            var tgl1 = $("input[name='tgl1']").val();
            var tgl2 = $("input[name='tgl2']").val();
            var url = "<?php echo site_url('keuangan/excel_feesharing')?>/"+tgl1+"/"+tgl2;
            window.location = url;
        });
        $(".cetakdetail").click(function(){
            ...
            var url = "<?php echo site_url('keuangan/cetakdetail_feesharing')?>/"+tgl1+"/"+tgl2+"/"+id_dokter+"/"+gol_pasien+"/"+pelayanan;
            openCenteredWindow(url);
        });
        $(".exceldetail").click(function(){
            ...
            var pelayanan = $("[name='pelayanan']").val();
            var id_dokter = $("[name='id_dokter']").val();
            var url = "<?php echo site_url('keuangan/exceldetail_feesharing')?>/"+tgl1+"/"+tgl2+"/"+id_dokter+"/"+gol_pasien+"/"+pelayanan;
            window.location = url;
        });
         $(".search").click(function(){
            ...
            window.location = "<?php echo site_url("keuangan/feesharing");?>/"+tgl1+"/"+tgl2;
        });
        $("[name='gol_pasien'], [name='pelayanan']").change(function(){
            var id_dokter = $("[name='id_dokter']").val();
            listpasien(id_dokter);
        })
    ...
    function listpasien(id_dokter){
        ...
        var td = <?php echo json_encode($q["tarifdokter"]) ?>;
        var tu = <?php echo json_encode($q["tarifdokterumum"]) ?>;
        var namatd = <?php echo json_encode($q["namatarifdokter"]) ?>;
        var gp = <?php echo json_encode($q["golpas"]) ?>;
        var html = "";
        var i = 1;
        var total = 0;
        var n = 0;
        console.log(row[id_dokter]);
        $.each(row[id_dokter],function(key,value){
            $.each(value,function(ktarif,val){
                ...
                var persen = (parseInt(val.tarif_bpjs)/parseInt(tarif_rs)*100);
                ...
                if (pelayanan=="all"){
                    if (gol_pasien=="all"){
                        ...
                        html += "<td class='text-right'>"+number_format(parseInt(val.tarif_bpjs),0)+"</td>";
                        html += "<td class='text-right'>"+number_format(parseInt(tarif_rs),0)+"</td>";
                        html += "<td class='text-right'>"+number_format(parseInt(tr[ktarif][key]),0)+"</td>";
                        html += "<td class='text-center'>"+(persen>100 ? "100" : persen.toFixed(2))+" %</td>";
                        var prs = persen>100 ? "100" : persen.toFixed(2);
                        // if (namatd[ktarif]=="PEMERIKSAAN DOKTER" && str_golpas=="UMUM"){
                        if (str_golpas=="UMUM"){
                            var bruto = (parseInt(tr[ktarif][key])*parseFloat(prs)/100)*parseFloat(tu[ktarif])/100;
                        } else
                            var bruto = (parseInt(tr[ktarif][key])*parseFloat(prs)/100)*parseFloat(td[ktarif])/100;
        ...
        <div class="box-body">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-striped table-hover" width="100%">
                    <thead>
                        <tr class="bg-navy">
                            <th class="text-center" style="vertical-align:middle" rowspan="2">ID Dokter</th>
                            <th class="text-center" style="vertical-align:middle" rowspan="2">Nama Dokter</th>
