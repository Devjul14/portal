	...
	$(document).ready(function(e){
        ...
        $(".print").click(function(){
         ...
         $(".search").click(function(){
            ...
            window.location = "<?php echo site_url("keuangan/pointsharing");?>/"+tgl1+"/"+tgl2;
        });
        ...
        $(".cetakdetail").click(function(){
            var tgl1 = $("input[name='tgl1']").val();
            var tgl2 = $("input[name='tgl2']").val();
            var url = "<?php echo site_url('keuangan/cetakdetail_point')?>/"+tgl1+"/"+tgl2;
            openCenteredWindow(url);
        });
        $(".exceldetail").click(function(){
            var tgl1 = $("input[name='tgl1']").val();
            var tgl2 = $("input[name='tgl2']").val();
            var url = "<?php echo site_url('keuangan/exceldetail_point')?>/"+tgl1+"/"+tgl2;
            openCenteredWindow(url);
        });
    });  
    function listpasien(){
        ...
        var html = "";
        $.ajax({
            url : "<?php echo base_url();?>keuangan/getpointsharing",
            method : "POST",
            ...
            success: function(data){
                ...
                var total_dokter_ralan = 0;
                var total_perawat_ralan = 0;
                var total_administrasi_ralan = 0;
                var total_bhp_ralan = 0;
                var total_rumahsakit_ralan = 0;

                var total_dokter_ranap = 0;
                var total_perawat_ranap = 0;
                var total_administrasi_ranap = 0;
                var total_bhp_ranap = 0;
                var total_rumahsakit_ranap = 0;
                var semua = 0;
                var semua_total = 0;
                ...
                $.each(row["tarif"],function(key,value){
                    ...
                    html += "<td class='text-right'>"+(value.dokter!=undefined ? number_format(parseInt(value.dokter)*jumlah/100,0) : 0)+"</td>";
                    html += "<td class='text-right'>"+(value.perawat!=undefined ? number_format(parseInt(value.perawat)*jumlah/100,0) : 0)+"</td>";
                    html += "<td class='text-right'>"+(value.administrasi!=undefined ? number_format(parseInt(value.administrasi)*jumlah/100,0) : 0)+"</td>";
                    html += "<td class='text-right'>"+(value.bhp!=undefined ? number_format(parseInt(value.bhp)*jumlah/100,0) : 0)+"</td>";
                    html += "<td class='text-right'>"+(value.rumahsakit!=undefined ? number_format(parseInt(value.rumahsakit)*jumlah/100,0) : 0)+"</td>";
                    html += "<td class='text-right'>"+(number_format(subtotal,0))+"</td>";
                    html += "</tr>";
                    total += parseInt(subtotal);
                    total_jumlah += parseInt(jumlah);
                    total_dokter_ralan += (value.dokter!=undefined ? parseInt(value.dokter)*jumlah/100 : 0);
                    total_perawat_ralan += (value.perawat!=undefined ? parseInt(value.perawat)*jumlah/100 : 0);
                    total_administrasi_ralan += (value.administrasi!=undefined ? parseInt(value.administrasi)*jumlah/100 : 0);
                    total_bhp_ralan += (value.bhp!=undefined ? parseInt(value.bhp)*jumlah/100 : 0);
                    total_rumahsakit_ralan += (value.rumahsakit!=undefined ? parseInt(value.rumahsakit)*jumlah/100 :0);
                });
                semua += total_jumlah;
                semua_total += total;
                html += "<tr class='bg-orange'>";
                ...
                html += "<td colspan='9' style='border-color:white'>&nbsp;</td>";
                html += "</tr>";
                html += "<tr class='bg-orange'>";
                html += "<td class='text-bold' colspan='9'>RAWAT INAP</td>";
                html += "</tr>";
                $.each(row["tarifranap"],function(key,value){
                    var jumlah = parseInt(row["jumlahranap"][key]);
                    subtotal = (value.dokter!=undefined ? value.dokter*jumlah/100 : 0)+
                    (value.perawat!=undefined ? value.perawat*jumlah/100 : 0)+
                    (value.administrasi!=undefined ? value.administrasi*jumlah/100 : 0)+
                    (value.bhp!=undefined ? value.bhp*jumlah/100 : 0)+
                    ...
                    total += parseInt(subtotal);
                    total_jumlah += parseInt(jumlah);
                    total_dokter_ranap += (value.dokter!=undefined ? parseInt(value.dokter)*jumlah/100 : 0);
                    total_perawat_ranap += (value.perawat!=undefined ? parseInt(value.perawat)*jumlah/100 : 0);
                    total_administrasi_ranap += (value.administrasi!=undefined ? parseInt(value.administrasi)*jumlah/100 : 0);
                    total_bhp_ranap += (value.bhp!=undefined ? parseInt(value.bhp)*jumlah/100 : 0);
                    total_rumahsakit_ranap += (value.rumahsakit!=undefined ? parseInt(value.rumahsakit)*jumlah/100 :0);
                });
                semua += total_jumlah;
                semua_total += total;
                html += "<tr class='bg-orange'>";
