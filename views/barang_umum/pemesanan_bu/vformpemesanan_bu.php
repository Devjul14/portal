<script>
    $(document).ready(function(){
        ...
        $(".back").click(function(){
            window.location = "<?php echo site_url('pemesanan_bu/pemesanan')?>";
            return false;
        });
        $(".hapus_item").click(function(){
            var kode_bu = $(this).attr("href");
            $("[name='kode_hapus']").val(kode_bu);
            $(".modal").show();
        });
        ...
        $(".ya").click(function(){
            var kode_bu       = $("[name='kode_hapus']").val();
            var no_pemesanan    = $("[name='no_pemesanan']").val()
            var no_permintaan   = $("[name='no_permintaan']").val()
            window.location     = "<?php echo site_url('pemesanan_bu/hapusitem_pemesanan')?>/"+no_permintaan+"/"+no_pemesanan+"/"+kode_bu;
            return false;
        });
        $("select[name='no_permintaan']").change(function(){
            var kode= $(this).val();
            window.location = "<?php echo site_url('pemesanan_bu/formpemesanan_bu')?>/"+kode;
            return false;
        });
        $("[name='nama']").typeahead({
            source: function(query, process) {
                ...
                map = {};
                var data = $.ajax({
                    url : "<?php echo base_url();?>pemesanan_bu/getbu_permintaan/"+no_permintaan,
                    method : "POST",
                    async: false,
                    data : {nama: query}
                }).responseText;
...
<div class="col-md-12">
    ...
    <div class="box box-info">
        <?php echo form_open("pemesanan_bu/simpanpemesanan_bu/".$action,array("id"=>"formsimpan","class"=>"form-horizontal"));?>
        <div class="box-body">
            ...
            <div class="form-group">
                ...
                <div class="col-md-10">
                   <textarea class="form-control" name="keterangan_pemesanan" <?php echo $a ?> ><?php echo $keterangan_pemesanan ?></textarea>
                </div>
            </div>
        </div>
    ...
    <div class="box box-info">
        <?php echo form_open("pemesanan_bu/simpanitempemesanan_bu",array("id"=>"formsimpan","class"=>"form-horizontal"));?>
                    ...
                    foreach ($q->result() as $value) {
                        echo "
                            <tr>
                                ...
                                <td>".number_format(($value->harga)*($value->jumlah*$value->isi),0,',','.')."</td>
                                <td>
                                    <button class='hapus_item btn btn-danger' type='button' href='".$value->kode_bu."'><i class='fa fa-minus'></i></button>
                                </td>
                            </tr>
                        ";
                    }
