...
var mywindow;
    ...
    $(document).ready(function(){
        ...
        setInterval(function(){ 
            ...
            view_mtbm(tgl1,tgl2,baris,hal);
            notif_mtbm(tgl1,tgl2,baris,hal);
        }, delay);
    ...
    var view_mtbm = function(tgl1,tgl2,baris,hal){
        var arrayData = {tgl1: tgl1, tgl2: tgl2, baris: baris, hal: hal};
        $.ajax({
            url: "<?php echo site_url('kia/view_mtbm');?>", 
            type: 'POST', 
            data: arrayData, 
            success: function(hasil){
                $(".view_mtbm").html(hasil);
            }
        });
    };
    var notif_mtbm = function(tgl1,tgl2,baris,hal){
        var arrayData = {tgl1: tgl1, tgl2: tgl2, baris: baris, hal: hal};
        $.ajax({
            url: "<?php echo site_url('kia/notif_mtbm');?>", 
            type: 'POST', 
            data: arrayData, 
            success: function(jmlrec){
                ...
                if (jmlrec>0){
                    $(".jmlrec").html(jmlrec);
                    $(document).attr("title", "("+jmlrec+") "+title);
                    $('.notif_mtbm').fadeIn('slow').delay(3000).fadeOut(100);
                    $('#chatAudio')[0].play();
                    $(".notif_mtbm").html(message);
                } else {
                    $(document).attr("title", title);
...
<div class='notif_mtbm'></div>
<div class="col-xs-12">
	<?php echo $this->session->flashdata('message');?>
	<?php echo form_open("kia/mtbm",array("id"=>"formcari"));?>
