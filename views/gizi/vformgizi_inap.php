 ...
 var mywindow1;
    ...
    $(document).ready(function(){
        ...
        $("[name='tindakan']").change(function(){
            ...
            var dokter_gizi= $("[name='dokter_gizi']").val();
            var petugas_gizi= $("[name='petugas_gizi']").val();
            var dokter_pengirim= $("[name='pengirim']").val();
            var tindakan= $(this).val();
            // alert(radiografer);
            $.ajax({
                url : "<?php echo base_url();?>gizi/addtindakan_inap",
                method : "POST",
                data : {no_reg: no_reg, kode_kelas: kode_kelas, tindakan: tindakan,tanggal: tanggal, pemeriksaan: pemeriksaan,dokter_gizi:dokter_gizi,petugas_gizi:petugas_gizi,dokter_pengirim:dokter_pengirim},
        ...
        $('.dataChange').click(function(evt) {
            ...
            if (jenis=='petugas_gizi'){
                ...
                var result = getpetugas_gizi(petugas_gizi);
                var dataInputField = $(result);
            }
            else
    ...
    var changeData = function(value,id,jenis){
        var no_reg= $("[name='no_reg']").val();
        $.ajax({
            url: "<?php echo site_url('gizi/changedata');?>/"+jenis, 
            type: 'POST', 
    ...
    function getdokter(val){
        var result = false;
        $.ajax({
            url: "<?php echo site_url('gizi/getdokter_pa');?>", 
            type: 'POST',
            async: false, 
            success: function(data){
        ...
        <div class="box-body">
        	...
        	<div class="form-horizontal">
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-2">
                        <select class="form-control" name="dokter_gizi">
                            <?php
                                foreach ($d->result() as $dk) {
                                    echo "
                                        <option value='".$dk->id_dokter."' ".($dk->id_dokter==$row->dokter_poli ? "selected" : "")." data-id='".$dk->gizi."'>".$dk->nama_dokter."</option>
                                    ";
                                }
                            ?>
                        </select>
                        ...
                        <select class="form-control" name="petugas_gizi">
                            <?php
                                foreach ($r->result() as $rg) {
                                    echo "
                                        <option value='".$rg->nip."' ".($rg->nip==$row->petugas_gizi ? "selected" : "").">".$rg->nama."</option>
                                    ";
                                }
                            ?>
                        </select>
