 ...
 var mywindow1;
    ...
    $(document).ready(function(){
        ...
        $("[name='tindakan']").change(function(){
            ...
            var petugas_gizi= $("[name='petugas_gizi']").val();
            var dokter_gizi= $("[name='dokter']").val();
            var ukuran_foto= $("[name='ukuran_foto']").val();
            var no_foto= $("[name='no_foto']").val();
            var dokter_pengirim= $("[name='pengirim']").val();
            $.ajax({
                url : "<?php echo base_url();?>gizi/addtindakan",
                method : "POST",
                data : {no_reg: no_reg, jenis: jenis, tindakan: tindakan, petugas_gizi: petugas_gizi,dokter_gizi:dokter_gizi,ukuranfoto: ukuran_foto,nofoto: no_foto,dokter_pengirim:dokter_pengirim},
        ...
        $('.hapus').click(function(){
            var id= $(this).attr("id");
            $.ajax({
                url : "<?php echo base_url();?>gizi/hapusralan",
                method : "POST",
                data : {id: id},
                success: function(data){
                     location.reload();
        ...
        $('.dataChange').click(function(evt) {
            ...
            if (dataText.hasClass("petugas_gizi")){
                jenis = "petugas_gizi";
            }
            if (jenis=='petugas'){
            ...
            if (jenis=='petugas_gizi'){
                var petugas_gizi = dataText.attr('petugas_gizi');
                var result = getpetugasgizi(petugas_gizi);
                var dataInputField = $(result);
            }
            else
                var dataContent = dataText.text().trim();
            dataText.before(dataInputField).hide();
            if (jenis=='petugas' || jenis=='petugas_gizi' || jenis=='dokter_pengirim'){
    ...
    var changeData = function(value,id,jenis){
        $.ajax({
            url: "<?php echo site_url('gizi/changedata_ralan');?>/"+jenis, 
            type: 'POST', 
    ...
    function getdokter(val){
        var result = false;
        $.ajax({
            url: "<?php echo site_url('gizi/getdokter_gizi');?>", 
            type: 'POST',
            async: false, 
            success: function(data){
    ...
    function getpetugasgizi(val){
        var result = false;
        $.ajax({
            url: "<?php echo site_url('gizi/getpetugas_gizi');?>", 
            type: 'POST',
            async: false, 
            success: function(data){
                var html = "<select name='petugas_gizi' class='selectpetugasgizi form-control'>";
                html += "<option value=''>---Pilih Petugas Gizi---</option>";
                $.each(JSON.parse(data), function(key, value){
...
<div class="col-md-12">
    <div class="box box-primary">
        <?php
            // echo form_open("gizi/simpangizi",array("id"=>"formsave","class"=>"form-horizontal"));
        ?>
            ...
            <div class="box-body">
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-2">
                        ...
                        <input type="hidden" class="form-control" name='gizi' readonly value="<?php echo $row->gizi;?>"/>
                    </div>
                    <label class="col-md-1 control-label">Petugas Gizi</label>
                    <div class="col-md-2">
                        <!-- <input type="text" class="form-control"  name='petugas_gizi' value="<?php echo $row->petugas_gizi;?>"/> -->
                        <select class="form-control" name="petugas_gizi">
                            <?php
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="hidden" readonly class="form-control" name='gizi' readonly value="<?php echo $row->gizi;?>"/>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-md-2 control-label">Diagosa Medis</label>
                    <div class="col-md-5">
            ...
            <table class="table table-bordered table-hover " id="myTable" >
                <thead>
                    <tr class="bg-navy">
                        ...
                        <th class="text-center">Petugas Gizi</th>
                        <th class="text-center">Diagosa Medis</th>
                ...
                <tbody>
                    <?php
                        ...
                        foreach($k->result() as $data){
                            ...
                            echo "<td class='text-left'><a href='#' class='petugas_gizi dataChange' id='".$data->id."' petugas_gizi='".$data->analys."'>".($data->analys=="" ? "---Pilih Petugas Gizi---" : (isset($petugas_gizi[$data->analys]) ? $petugas_gizi[$data->analys] : "---Pilih Petugas Gizi---") )."</a></td>";
