    ...
    $(document).ready(function(){
        ...
        $('.okpassword').click(function() {
            ...
            $.each($(".bg-gray"),function(key,value){
                multiid[$(this).attr("id")] = $(this).attr("id");
            });
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {id: id,value: value, jenis: jenis,password: password,multiid: multiid},
                success: function(){
                    location.reload();
                }
            });
        ...
        $('.dataChange').click(function(evt) {
            ...
            if (dataText.hasClass("pemeriksaan")){
                jenis = "pemeriksaan";
            }
            else
        ...
        if (jenis=="analys"){
            ...
            if (value!="")
                ...
                $.ajax({
                    url: "<?php echo site_url('lab/changedata_ralan');?>",
                    type: 'POST',
                    data: {id: id, multiid: multiid,value: value, jenis: jenis},
                    success: function(){
                        location.reload();
                    }
                });
                        ...
                        <select class="form-control" name="anal">
                            <?php
                                foreach ($r->result() as $rg) {
                                    echo "
                                        <option value='".$rg->id_perawat."' ".($rg->nip==$row->id_perawat ? "selected" : "").">".$rg->nama_perawat."</option>
                                    ";
                                }
                            ?>
                        </select>
    ...
    <div class="box box-primary">
        <div class="box-body">
            <table class="table table-bordered table-hover " id="myTable" >
                ...
                <tbody>
                    <?php
                        ...
                        foreach($k->result() as $data){
                            ...
                            echo "<tr class='data' id='".$data->id."'>";
                            echo "<td>".$i++."</td>";
                            echo "<td>".$data->nama_tindakan." ".(!isset($kr[$data->kode_tindakan]) || trim($kr[$data->kode_tindakan])=="" ? "<div class='pull-right'><button id='".$data->id."' class='hapus btn btn-sm btn-danger'><i class='fa fa-minus'></i></div>" : ($kr[$data->kode_tindakan]=="" ? "<div class='pull-right'><button id='".$data->id."' class='hapus btn btn-sm btn-danger'><i class='fa fa-minus'></i></div>" : ""))."</td>";
                            ...
                            echo "<td class='text-center'><a href='#' class='pemeriksaan dataChange' id='".$data->id."'>".($data->pemeriksaan!="" ? $data->pemeriksaan : "-" )."</a></td>";
                            echo "<td class='text-left'><a href='#' class='metode_swab dataChange' id='".$data->id."' metode_swab='".$data->metode_swab."'>".($data->metode_swab=="" ? "---Metode Swab---" : (isset($ms[$data->metode_swab]) ? $ms[$data->metode_swab] : "---Metode Swab---") )."</a></td>";
                            echo "<td class='text-right'><a href='#' class='dataChange' id='".$data->id."'>".number_format($data->jumlah,0,'.','.')."</a></td>";
                            echo "<td class='text-center'>".($data->kode_tindakan=="L158" ? "<a href='#' class='kode dataChange' id='".$data->id."'>".($data->kode ? $data->kode : "Masukan Kode")."</a>" : "")."</td>";
                            echo "</tr>";
                        }
                    ?>
                </tbody>
                <tfoot>
                    <tr>
                        <th colspan="6" style="vertical-align: middle" ><span class="pull-right">Subtotal</span></th>
                        <th colspan="4" style="vertical-align: middle" ><input type="text" readonly name="subtotal" class="form-control text-right" value="<?php echo number_format($subtotal,0,',','.');?>"></th>
                    </tr>
                    <tr>
                        <th colspan="6" style="vertical-align: middle" ><span class="pull-right">Disc</span></th>
                        <th colspan="3" width="250px" style="vertical-align: middle" >
                            <div class="row">
                                <div class="col-sm-5">
                    ...
                    <?php if ($row->status_bayar=="TAGIH"): ?>
                        <tr>
                            <th colspan="6" style="vertical-align: middle" ><span class="pull-right">Sharing</span></th>
                            <th colspan="3" style="vertical-align: middle" ><input type="text" readonly name="sharing" class="form-control text-right" value="<?php echo number_format($sharing,0,',','.');?>"></th>
                        </tr>
                    <?php else: ?>
                        <input type="hidden" name="sharing">
                    <?php endif ?>
                    <tr>
                        <th colspan="6" style="vertical-align: middle" ><?php echo $tgl_pembayaran;?><span class="pull-right">Total</span></th>
                        ...
                        <td class="text-center" colspan="6">
                            <?php echo form_open("lab/addtindakan",array("id"=>"formsave","class"=>"form-horizontal"));?>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-3">
                                        <input type="text" name="pemeriksaan" placeholder="Pemeriksaan ke-" class="form-control">
                                    </div>
                                    <div class="col-md-2">
