<script>
    ...
    $(document).ready(function(e) {
        ...
        $(".cetaklistsprint_keluar").click(function() {
            var tgl1 = $("[name='list_tgl1']").val();
            var tgl2 = $("[name='list_tgl2']").val();
            var url = "<?php echo site_url('suket/cetaklistsprint_keluar') ?>/" + tgl1 + "/" + tgl2;
            openCenteredWindow(url);
            return false;
        });
        $(".listsprint_keluar").click(function() {
            $(".modallistsprint_keluar").modal("show");
            return false;
        });
        $(".add").click(function() {
            window.location = "<?php echo site_url('suket/addsprint_keluar/') ?>";
            return false;
        });
        $(".edit").click(function() {
            var id = $(".bg-gray").attr("no_sprint");
            window.location = "<?php echo site_url('suket/addsprint_keluar') ?>/" + id;
            return false;
        });
        ...
        $(".reset").click(function() {
            var url = "<?php echo site_url('suket/resetno_sprint_keluar'); ?>";
            window.location = url;
        });
        $(".lampiran").click(function() {
            var id = $(".bg-gray").attr("no_sprint");
            var url = "<?php echo site_url('suket/lampiran_sprint'); ?>/" + id;
            window.location = url;
            return false;
        });
        $(".lampiran_kegiatan").click(function() {
            var id = $(".bg-gray").attr("no_sprint");
            var url = "<?php echo site_url('suket/lampiran_kegiatan'); ?>/" + id;
            window.location = url;
            return false;
        });
    });
    ...
    function pencarian() {
        ...
        $.ajax({
            ...
            url: "<?php echo site_url('suket/getcarino_sprint_keluar'); ?>",
            success: function(result) {
                window.location = "<?php echo site_url('suket/listsprin_keluar'); ?>";
            },
            ...
            <table class="table table-bordered table-hover " id="myTable">
                <thead>
                    <tr class="bg-navy">
                        ...
                        <th class='text-center'>No. Sprint Keluar</th>
                ...
                <tbody>
                    ...
                    foreach ($q3->result() as $row) {
                        echo "<tr id=data no_sprint='" . $row->no_sprint . "' tanggal='" . date("d/m/Y", strtotime($row->tanggal)) . "' no_sprint='" . $row->no_sprint . "' asal_surat='" . $row->asal_surat . "' tmt='" . date("d/m/Y", strtotime($row->tmt)) . "' perihal='" . $perihal . "' ket='" . $ket . "'>";
                        ...
                        echo "<td class='text-center'>" . date("d-m-Y", strtotime($row->tmt)) . "</td>";
        ...
        <div class="box-footer">
            <div class="pull-left">
                ...
                <div class="btn-group">
                    ...
                    <button class="listsprint_keluar btn btn-info" type="button"><i class="fa fa-file"></i> Rekap</button>
                    <button class="lampiran btn bg-purple" type="button"><i class="fa fa-paperclip"></i> Lampiran</button>
                    <button class="lampiran_kegiatan btn bg-maroon" type="button"><i class="fa fa-paperclip"></i> Lampiran Kegiatan</button>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <div class="form-group">
                        <div class="col-sm-12">
                            <div class="input-group">
                                <input class="form-control" type="text" name="cari_no" placeholder="Tanggal/ No. Sprint Keluar" />
...
<div class='modal modallistsprint_keluar no-print' role="dialog">
    <div class='modal-dialog'>
        <div class='modal-content'>
            <div class="modal-header bg-orange">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                <h4 class='modal-title'><i class="icon fa fa-warning"></i>&nbsp;&nbsp;Rekap Sprint Keluar</h4>
            </div>
            <div class='modal-body'>
                <div class="row">
                    <div class="col-sm-12">
            ...
            <div class='modal-footer'>
                <div class="pull-right">
                    <button class="cetaklistsprint_keluar btn btn-success">Cetak</button>
                </div>
            </div>
        </div>
    </div>
