<script>
    ...
    function pencarian() {
        ...
        $.ajax({
            ...
            success: function(result) {
                window.location = "<?php echo site_url('suket/listberitalepaskeperawatan'); ?>";
            },
    ...
    $(document).ready(function(e) {
        ...
        $(".cetaklistkelahiran").click(function() {
            ...
            var url = "<?php echo site_url('suket/cetaklistsuratistirahatsakit') ?>/" + tgl1 + "/" + tgl2;
            openCenteredWindow(url);
            return false;
        });
        ...
        $(".reset").click(function() {
            window.location = "<?php echo site_url('suket/reset_surat_istirahat_sakit'); ?>/";
            return false;
        });
        ...
        $(".detailkelahiran").click(function() {
            var no_rm = $(".bg-gray").attr("href");
            var no_reg = $(".bg-gray").attr("no_reg");
            var url = "<?php echo site_url('suket/detailkelahiran') ?>/" + no_rm + "/" + no_reg;
            window.location = url;
            return false;
        });
        $(".detailsuratistirahatsakit").click(function() {
            var no_rm = $(".bg-gray").attr("href");
            var no_reg = $(".bg-gray").attr("no_reg");
        ...
        $(".cetaksuratistirahatsakit").click(function() {
            var no_rm = $(".bg-gray").attr("href");
            var no_reg = $(".bg-gray").attr("no_reg");
            var jenis = $(".bg-gray").attr("jenis");
            var url = "<?php echo site_url('surat/suratistirahatsakit'); ?>/" + no_reg + "/" + no_rm + "/"+ jenis;
            openCenteredWindow(url)
        })
            ...
            <table class="table table-bordered table-hover " id="myTable">
                <thead>
                    <tr class="bg-navy">
                        ...
                        <th class='text-center'>Selama</th>
                        <th class='text-center'>Alamat dituju</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                        ...
                        foreach ($q3["surat"] as $key => $row) {
                              ...
                              if ($data->nama_pasien!=""){
                                ...
                                $nomor_surat = $row->nomor_surat . "/ IST/ " . $bulan[(int)(date("m", strtotime($row->tgl_insert)))] . "/ " . date("Y", strtotime($row->tgl_insert));
                                echo "<tr id=data jenis='" . $row->jenis . "' href='" . $row->no_rm . "' berat_badan='" . $data->berat_badan . "' petugas_rm='" . $petugas_rm . "' no_reg_encrypt='" . $row->no_reg . "' telpon='" . $telpon . "' id_dokter='" . $data->dokter . "' no_reg='" . $row->no_reg . "' no_bpjs='" . $data->no_bpjs . "' kode_kamar='" . $data->kode_kamar . "' kode_kelas='" . $data->kode_kelas . "' kode_ruangan='" . $data->kode_ruangan . "' no_bed='" . $data->no_bed . "' no_sep='" . $data->no_sjp . "' tgl_masuk='" . date("Y/m/d", strtotime($data->tgl_masuk)) . "'>";
                                ...
                                echo "<td>" . ($row->mulai == "" ? "" : date("d-m-Y", strtotime($row->mulai))) . "</td>";
                                echo "<td>" . ($row->sampai == "" ? "" : date("d-m-Y", strtotime($row->sampai))) . "</td>";
                                echo "<td>" . $row->selama . "</td>";
            ...
            <div class="row">
                <div class="col-xs-12">
                    <div class="pull-right">
                        ...
                        <button class="cetaksuratistirahatsakit btn btn-sm btn-info" type="button"><i class="fa  fa-print"></i> Cetak</button>
                        <button class="detailsuratistirahatsakit btn btn-sm btn-success" type="button"><i class="fa  fa-search-plus"></i> Detail</button>
                    </div>
                </div>
            </div>
        </div>
...
<div class='modal modallistkelahiran no-print' role="dialog">
    <div class='modal-dialog'>
        <div class='modal-content'>
            <div class="modal-header bg-orange">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                <h4 class='modal-title'><i class="icon fa fa-warning"></i>&nbsp;&nbsp;Cetak Surat Istirahat Sakit</h4>
            </div>
            <div class='modal-body'>
                <div class="row">
                    <div class="col-sm-12">
