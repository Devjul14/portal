<script>
    ...
    $(document).ready(function(e) {
        ...
        $(".cetaklistkelahiran").click(function() {
            ...
            var url = "<?php echo site_url('suket/cetaklistkelahiran') ?>/" + tgl1 + "/" + tgl2;
            openCenteredWindow(url);
            return false;
        });
        ...
        $(".reset").click(function() {
            window.location = "<?php echo site_url('suket/reset_pasienkelahiran'); ?>/";
            return false;
        });
        ...
        $(".detailkelahiran").click(function() {
            ...
            var url = "<?php echo site_url('suket/detailkelahiran') ?>/" + no_rm + "/" + no_reg + "/" + jenis;
            window.location = url;
            return false;
        });
    ...
    <div class="box box-primary">
            ...
            <table class="table table-bordered table-hover " id="myTable">
                ...
                <tbody>
                    ...
                    foreach ($q3->result() as $row) {
                        ...
                        $nomor_surat = $row->nomor_surat . "/ SKK/ " . $bulan[(int)(date("m", strtotime($row->tgl_lahir)))] . "/ " . date("Y", strtotime($row->tgl_lahir));
                        echo "<tr id=data jenis='" . $row->jenis . "' href='" . $row->no_rm . "' berat_badan='" . $row->berat_badan . "' petugas_rm='" . $petugas_rm . "' no_reg_encrypt='" . $row->no_reg . "' telpon='" . $telpon . "' id_dokter='" . $row->dokter . "' no_reg='" . $row->no_reg . "' no_bpjs='" . $row->no_bpjs . "' kode_kamar='" . $row->kode_kamar . "' kode_kelas='" . $row->kode_kelas . "' kode_ruangan='" . $row->kode_ruangan . "' no_bed='" . $row->no_bed . "' no_sep='" . $row->no_sjp . "' tgl_masuk='" . date("Y/m/d", strtotime($row->tgl_masuk)) . "'>";
        ...
        <div class="box-footer">
            <div class="row">
                <div class="col-xs-12">
                    <div class="pull-right">
                        <button class="listkelahiran btn btn-sm btn-primary" type="button"><i class="fa  fa-print"></i> Cetak</button>
                        <button class="detailkelahiran btn btn-sm btn-success" type="button"><i class="fa  fa-search-plus"></i> Detail</button>
                    </div>
                </div>
            </div>
        </div>
...
<div class='modal modallistkelahiran no-print' role="dialog">
    <div class='modal-dialog'>
        <div class='modal-content'>
            <div class="modal-header bg-orange">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                <h4 class='modal-title'><i class="icon fa fa-warning"></i>&nbsp;&nbsp;Cetak Surat Kelahiran</h4>
            </div>
            <div class='modal-body'>
                <div class="row">
                    <div class="col-sm-12">
