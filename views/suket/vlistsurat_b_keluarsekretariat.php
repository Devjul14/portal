<script>
    ...
    $(document).ready(function(e) {
        ...
        $(".cetaklist_bkeluar").click(function() {
            var tgl1 = $("[name='list_tgl1']").val();
            var tgl2 = $("[name='list_tgl2']").val();
            var url = "<?php echo site_url('suket/cetaklist_bkeluar') ?>/" + tgl1 + "/" + tgl2;
            openCenteredWindow(url);
            return false;
        });
        $(".list_bkeluar").click(function() {
            $(".modallist_bkeluar").modal("show");
            return false;
        });
        $(".add").click(function() {
            window.location = "<?php echo site_url('suket/addsurat_b_keluarsekretariat/') ?>";
            return false;
        });
        $(".edit").click(function() {
            var id = $(".bg-gray").attr("no_b_keluar");
            window.location = "<?php echo site_url('suket/addsurat_b_keluarsekretariat') ?>/" + id;
            return false;
        });
        ...
        $(".reset").click(function() {
            var url = "<?php echo site_url('suket/resetno_b_keluar'); ?>";
            window.location = url;
        });
    });
    ...
    function pencarian() {
        ...
        $.ajax({
            ...
            url: "<?php echo site_url('suket/getcarino_b_keluar'); ?>",
            success: function(result) {
                window.location = "<?php echo site_url('suket/listsurat_b_keluarsekretariat'); ?>";
            },
            ...
            <table class="table table-bordered table-hover " id="myTable">
                <thead>
                    <tr class="bg-navy">
                        ...
                        <th class='text-center'>No. B. Keluar</th>
                ...
                <tbody>
                    ...
                    foreach ($q3->result() as $row) {
                        echo "<tr id=data no_b_keluar='" . $row->no_b_keluar . "' tanggal='" . date("Y/m/d", strtotime($row->tanggal)) . "' no_b_keluar='" . date("Y/m/d", strtotime($row->no_b_keluar)) . "' kepada='" . $row->kepada . "' asal_surat='" . $row->asal_surat . "' perihal='" . $perihal . "' lampiran='" . $lampiran . "'>";
                        echo "<td class='text-center'>" . date("d-m-Y", strtotime($row->tanggal)) . "</td>";
                        echo "<td class='text-center'>" . $row->no_b_keluar . "</td>";
        ...
        <div class="box-footer">
            <div class="pull-left">
                ...
                <div class="btn-group">
                    ...
                    <button class="list_bkeluar btn btn-info" type="button"><i class="fa  fa-file"></i> Rekap</button>
                </div>
            </div>
        </div>
    </div>
                    ...
                    <div class="form-group">
                        <div class="col-sm-12">
                            <div class="input-group">
                                <input class="form-control" type="text" name="cari_no" placeholder="Tanggal/ No. B Keluar" />
...
<div class='modal modallist_bkeluar no-print' role="dialog">
    <div class='modal-dialog'>
        <div class='modal-content'>
            <div class="modal-header bg-orange">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                <h4 class='modal-title'><i class="icon fa fa-warning"></i>&nbsp;&nbsp;Cetak Surat B Keluar Sekretariat</h4>
            </div>
            <div class='modal-body'>
                <div class="row">
                    <div class="col-sm-12">
            ...
            <div class='modal-footer'>
                <div class="pull-right">
                    <button class="cetaklist_bkeluar btn btn-success">Cetak</button>
                </div>
            </div>
        </div>
    </div>
