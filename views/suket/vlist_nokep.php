<script>
    function pencarian() {
        ...
        $.ajax({
            ...
            url: "<?php echo site_url('suket/getcarino_nokep'); ?>",
            success: function(result) {
                window.location = "<?php echo site_url('suket/list_nokep'); ?>";
            },
    ...
    $(document).ready(function(e) {
        ...
        $(".cetak").click(function() {
            ...
            var url = "<?php echo site_url('suket/cetaklist_nokep') ?>/" + tgl1 + "/" + tgl2;
        ...
        $(".add").click(function() {
            window.location = "<?php echo site_url('suket/add_nokep/') ?>";
            return false;
        });
        $(".edit").click(function() {
            var id = $(".bg-gray").attr("no_surat");
            window.location = "<?php echo site_url('suket/add_nokep') ?>/" + id;
            return false;
        });
        ...
        $(".reset").click(function() {
            var url = "<?php echo site_url('suket/reset_nokep'); ?>";
            window.location = url;
        });
    });
            ...
            <table class="table table-bordered table-hover " id="myTable">
                <thead>
                    <tr class="bg-navy">
                        ...
                        <th class='text-center'>No. Kep</th>
                        <th width="15%" class='text-center'>Tanggal Kep</th>
                ...
                <tbody>
                    ...
                    foreach ($q3->result() as $row) {
                        echo "<tr id=data no_surat='" . $row->no_surat . "' tanggal='" . date("d/m/Y", strtotime($row->tanggal)) . "' tgl_kep='" . date("d/m/Y", strtotime($row->tgl_kep)) . "' uraian='" . $row->uraian . "' asal_surat='" . $row->asal_surat . "' perihal='" . $perihal . "' lampiran='" . $lampiran . "' ket='" . $ket . "'>";
                        $bulan = array("", "I", "II", "III", "IV", "V", "VI", "VII", "VIII", "IX", "X", "XI", "XII");
                        $no_surat = $row->no_surat . "/ KEP / " . $bulan[(int)(date("m", strtotime($row->tanggal)))] . "/ " . date("Y", strtotime($row->tanggal));

                        echo "<td class='text-center'>" . date("d-m-Y", strtotime($row->tanggal)) . "</td>";
                        echo "<td class='text-center'>" . $no_surat . "</td>";
                        echo "<td class='text-center'>" . date("d-m-Y", strtotime($row->tgl_kep)) . "</td>";
                    ...
                    <div class="form-group">
                        <div class="col-sm-12">
                            <div class="input-group">
                                <input class="form-control" type="text" name="cari_no" placeholder="Tanggal/ No. Surat" />
...
<div class='modal modalrekap no-print' role="dialog">
    <div class='modal-dialog'>
        <div class='modal-content'>
            <div class="modal-header bg-orange">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                <h4 class='modal-title'><i class="icon fa fa-warning"></i>&nbsp;&nbsp;Rekap No Kep</h4>
            </div>
            <div class='modal-body'>
                <div class="row">
                    <div class="col-sm-12">
