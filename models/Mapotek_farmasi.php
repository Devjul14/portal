<?php
class Mapotek_farmasi extends CI_Model{
   function __construct()
	...
	function changedata($change){
		$this->db->select("hrg_jual,hrg_obat_kronis");
		...
		if ($change=="qty"){
			...
			$this->db->set("harga_obat_kronis",$this->input->post("value")*$q->hrg_obat_kronis);
		}
    ...
    $this->db->select("a.no_reg,sum(harga_obat_kronis) as jumlah");
		$q = $this->db->get_where("apotek a", ["a.no_reg" => $this->input->post("no_reg")]);
    if ($q->num_rows()>0){
      $q = $q->row();
      $dat = array("obat_kronis"=>$q->jumlah);
      $this->db->where("no_reg",$this->input->post("no_reg"));
      $this->db->update("pasien_ralan",$dat);
    }
	}
    ...
    function getlaporanharian_ralan($tgl1,$tgl2){
    	$tgl1 = date("Y-m-d",strtotime($tgl1));
    	$tgl2 = date("Y-m-d",strtotime($tgl2));
    	$this->db->select("kode_obat,nama_obat,sum(apt.jumlah) as jumlah,sum(apt.qty) as qty,pr.tanggal,gp.jenis as gol");
    	$this->db->join("pasien_ralan pr","pr.no_reg=apt.no_reg","inner");
    	$this->db->join("gol_pasien gp","gp.id_gol=pr.gol_pasien","inner");
    	$this->db->where("depo","ralan");
    	$this->db->like("pr.tanggal",$tgl1);
    	$this->db->or_like("pr.tanggal",$tgl2);
    	$this->db->order_by("pr.tanggal","desc");
    	$this->db->group_by("kode_obat");
    	$q = $this->db->get("apotek apt");
    	return $q;
    }
    function getgolongan_ralan($tgl1,$tgl2){
    	$tgl1 = date("Y-m-d",strtotime($tgl1));
    	$tgl2 = date("Y-m-d",strtotime($tgl2));
    	$q = $this->getlaporanharian_ralan($tgl1,$tgl2);
    	foreach ($q->result() as $row) {
            if (isset($data[$row->kode_obat][$row->gol])){
                $data[$row->kode_obat][$row->gol] += $row->jumlah;
            } else {
                $data[$row->kode_obat][$row->gol] = $row->jumlah;
            }
    	}
    ...
    function getgolonganqty_ralan($tgl1,$tgl2){
    	$tgl1 = date("Y-m-d",strtotime($tgl1));
    	$tgl2 = date("Y-m-d",strtotime($tgl2));
    	$q = $this->getlaporanharian_ralan($tgl1,$tgl2);
    	foreach ($q->result() as $row) {
            if (isset($data[$row->kode_obat][$row->gol])){
                $data[$row->kode_obat][$row->gol] += $row->qty;
            } else {
                $data[$row->kode_obat][$row->gol] = $row->qty;
            }
    	}
    ...
    function getlaporanharian_inap($tgl1,$tgl2){
    	$tgl1 = date("Y-m-d",strtotime($tgl1));
    	$tgl2 = date("Y-m-d",strtotime($tgl2));

    	$this->db->select("kode_obat,nama_obat,sum(apt.jumlah) as jumlah,sum(apt.qty) as qty,apt.tanggal,gp.jenis as gol");
    	$this->db->join("pasien_inap pr","pr.no_reg=apt.no_reg","inner");
    	$this->db->join("pasien p","p.no_pasien=pr.no_rm");
    	$this->db->join("gol_pasien gp","gp.id_gol=p.id_gol","inner");
    	$this->db->where("depo","ranap");
    	$this->db->where("apt.tanggal>=",$tgl1);
    	$this->db->where("apt.tanggal<=",$tgl2);
    	$this->db->order_by("apt.tanggal","desc");
    	$this->db->group_by("kode_obat");
    	$q = $this->db->get("apotek_inap apt");
    	return $q;
    }
    function getgolongan_inap($tgl1,$tgl2){
    	$tgl1 = date("Y-m-d",strtotime($tgl1));
    	$tgl2 = date("Y-m-d",strtotime($tgl2));
    	$q = $this->getlaporanharian_inap($tgl1,$tgl2);
    	foreach ($q->result() as $row) {
            if (isset($data[$row->kode_obat][$row->gol])){
                $data[$row->kode_obat][$row->gol] += $row->jumlah;
            } else {
    ...
    function getgolonganqty_inap($tgl1,$tgl2){
    	$tgl1 = date("Y-m-d",strtotime($tgl1));
    	$tgl2 = date("Y-m-d",strtotime($tgl2));
    	$q = $this->getlaporanharian_inap($tgl1,$tgl2);
    	foreach ($q->result() as $row) {
    ...
    function getlaporanharian_igd($tgl1,$tgl2){
    	$tgl1 = date("Y-m-d",strtotime($tgl1));
    	$tgl2 = date("Y-m-d",strtotime($tgl2));
    	$this->db->select("kode_obat,nama_obat,sum(apt.jumlah) as jumlah,sum(apt.qty) as qty,pr.tanggal,gp.jenis as gol");
    	$this->db->join("pasien_ralan pr","pr.no_reg=apt.no_reg","inner");
    ...
    function getgolongan_igd($tgl1,$tgl2){
    	$tgl1 = date("Y-m-d",strtotime($tgl1));
    	$tgl2 = date("Y-m-d",strtotime($tgl2));
    	$q = $this->getlaporanharian_igd($tgl1,$tgl2);
    	foreach ($q->result() as $row) {
            if (isset($data[$row->kode_obat][$row->gol])){
                $data[$row->kode_obat][$row->gol] += $row->jumlah;
            } else {
    ...
    function getgolonganqty_igd($tgl1,$tgl2){
    	$tgl1 = date("Y-m-d",strtotime($tgl1));
    	$tgl2 = date("Y-m-d",strtotime($tgl2));
    	$q = $this->getlaporanharian_igd($tgl1,$tgl2);
    	foreach ($q->result() as $row) {
    ...
    function getlaporanharianinap_igd($tgl1,$tgl2){
    	$tgl1 = date("Y-m-d",strtotime($tgl1));
    	$tgl2 = date("Y-m-d",strtotime($tgl2));
    ...
    function getgolonganinap_igd($tgl1,$tgl2){
    	$tgl1 = date("Y-m-d",strtotime($tgl1));
    	$tgl2 = date("Y-m-d",strtotime($tgl2));
    	$q = $this->getlaporanharianinap_igd($tgl1,$tgl2);
    	foreach ($q->result() as $row) {
            if (isset($data[$row->kode_obat][$row->gol])){
                $data[$row->kode_obat][$row->gol] += $row->jumlah;
            } else {
    ...
    function getgolonganqtyinap_igd($tgl1,$tgl2){
    	$tgl1 = date("Y-m-d",strtotime($tgl1));
    	$tgl2 = date("Y-m-d",strtotime($tgl2));
    	$q = $this->getlaporanharianinap_igd($tgl1,$tgl2);
    	foreach ($q->result() as $row) {
