<?php
class Mpersonalia extends CI_Model
{
    ...
    function getpasien_rawatjalan()
    {
        ...
        $this->db->select("pr.*,p.nama_pasien as nama_pasien,p.jenis_kelamin as jenis_kelamin");
        $this->db->group_start();
    ...
    function getpasien_ralan($page, $offset)
    {
        ...
        $status_pasien = $this->session->userdata("status_pasien");
        $nama = $this->session->userdata("nama");
        $this->db->select("p.no_bpjs,p.telpon,pr.*,p.nama_pasien as nama_pasien, g.keterangan as gol_pasien,sp.keterangan as ket_pulang, p.jenis_kelamin as jenis_kelamin");
        $this->db->group_start();
        $this->db->like("p.no_pasien", $no_pasien);
        $this->db->or_like("no_reg", $no_pasien);
        $this->db->or_like("no_bpjs", $no_pasien);
        ...
        $this->db->order_by("pr.no_antrian,no_reg", "desc");
        $this->db->join("pasien p", "p.no_pasien=pr.no_pasien");
    ...
    function getpasien_ralan_detail($no_reg)
    {
        ...
        return $this->db->get_where("pasien_ralan", ["no_reg" => $no_reg])->row();
    }
    ...
    {
        // if ($this->input->post('baru')=='y') {
        // 	$no_kk = $this->getno_kk_baru();
        // } else {
        // 	$no_kk = $this->input->post('no_kk');
        // }
        // $no_pasien1 = $no_kk;
        // $no_pasien2 = $this->getno_pasien_baru($no_kk);
        // no_kk='".$no_kk."',
        ...
        switch ($action) {
            case 'simpan':
                ...
                break;
            case 'edit':
                $this->db->where("no_reg=no_pasien");
                $this->db->where("no_reg", $this->input->post('idlama'));
                $q = $this->db->get_where("pasien_ralan");
                if ($q->num_rows() > 0) {
                    ...
                    $this->db->where("no_pasien", $this->input->post('idlama'));
                    $this->db->update("pasien_ralan", ["no_pasien" => $no_pasien, "gol_pasien" => $this->input->post('gol_pas'), "status_bayar" => ($gp == "LUNAS" ? "" : $gp), "nama_pasien" => $this->input->post("nama_pasien")]);
                    $this->db->where("no_rm", $this->input->post('idlama'));
                    $this->db->update("pasien_triage", ["no_rm" => $no_pasien]);
                    $this->db->where("no_rm", $this->input->post('idlama'));
                    $this->db->update("pasien_igd", ["no_rm" => $no_pasien]);
                } else {
    ...
    function simpanlaporan_tindakaninap()
    {
        ...
        if (is_array($tindakan)) {
          foreach ($tindakan as $key => $value) {
              $tnd .= $koma . ($value != "" ? $value : "");
              $koma = ",";
          }
        } else {
          $tnd = $this->input->post("tindakan");
        }
        $data = array(
            'diagnosa' => $this->input->post("diagnosa"),
    ...
    function getjumlahperawat()
    {
        ...
        //  $this->db->like("nama_pasien",$this->session->flashdata("nama"));
        // }
        $this->db->like("nama_perawat", $this->session->userdata("no_pasien"));
        $this->db->or_like("id_perawat", $this->session->userdata("no_pasien"));
        $this->db->or_like("no_bpjs", $this->session->userdata("no_pasien"));
        $this->db->or_like("nip", $this->session->userdata("no_pasien"));
        $this->db->or_like("ktp", $this->session->userdata("no_pasien"));
    ...
    function getperawat()
    {
        $this->db->select("p.*,jtp.keterangan as namajenis_tenaga,k.keterangan");
        $this->db->join("jenis_tenaga_perawat jtp", "jtp.id_jtp=p.id_jtp", "left");
        $this->db->group_by("p.id_perawat,k.kode_pangkat");
        $this->db->join("pangkat k","k.kode_pangkat=p.id_pangkat","left");
        $this->db->like("nama_perawat", $this->session->userdata("id_perawat"));
        $this->db->or_like("id_perawat", $this->session->userdata("id_perawat"));
        $this->db->or_like("no_bpjs", $this->session->userdata("id_perawat"));
        $this->db->or_like("nip", $this->session->userdata("id_perawat"));
        $this->db->or_like("ktp", $this->session->userdata("id_perawat"));
        $this->db->order_by("id_perawat");
        $query = $this->db->get("perawat p");
        return $query;
    }

    function getdetailperawat($id_perawat){
        $q = $this->db->get_where("perawat",["id_perawat"=>$id_perawat]);
        return $q->row();
    }
    function getjenis_tenaga_perawat(){
        $query = $this->db->get("jenis_tenaga_perawat");
        return $query->result();
    }
    ...
    switch ($aksi) {
      case 'simpan' :
        $data = array(
          ...
          'tgl_str'       => date('Y-m-d', strtotime($this->input->post('tgl_str'))),
          'no_sip'        => $this->input->post('no_sip'),
          'tgl_sip'       => date('Y-m-d', strtotime($this->input->post('tgl_sip'))),
          'no_kta'        => $this->input->post('no_kta'),
          'npwp'          => $this->input->post('npwp'),
          'no_label'      => $this->input->post('no_label'),
          'no_pns'        => $this->input->post('no_pns'),
          'kode_pos'      => $this->input->post('kode_pos'),
          'email'         => $this->input->post('email'),
          'tb'            => $this->input->post('tb'),
          'bb'            => $this->input->post('bb'),
          'peci'          => $this->input->post('peci'),
          ...
          'suku'          => $this->input->post('suku'),
          'no_ktpa'       => $this->input->post('no_ktpa'),
          'no_karis'      => $this->input->post('no_karis'),
          'no_kpi'        => $this->input->post('no_kpi'),
          'no_regis'      => $this->input->post('no_regis'),
          'no_randis'     => $this->input->post('no_randis'),
        );
        $q = $this->getdetailperawat($this->input->post('id_perawat'));
        if ($q) {
          $msg  = "danger- Perawat sudah ada sebelumnya";
          return $msg;
        } else {
    ...
    {
        $this->db->select("jd.*, p.nama_perawat, r.nama_ruangan");
        $this->db->join("perawat p", "p.id_perawat = jd.id_perawat","inner");
        $this->db->join("ruangan r", "r.kode_bagian = jd.id_ruangan","inner");
        $this->db->group_by("r.kode_bagian,jd.id_perawat");
        $q = $this->db->get("jadwal_perawat jd");
        return $q;
    }
    function getperawat2()
    {
        $this->db->select("p.*,r.nama_ruangan,p.bagian as id_ruangan");
        $this->db->join("ruangan r", "r.kode_bagian = p.bagian");
        $this->db->group_by("r.kode_bagian");
        $q = $this->db->get("perawat p");
        return $q;
    }
    function getjadwalperawatdetail($id_perawat,$ruangan,$bulan,$tahun)
    {
        $this->db->order_by("id_perawat");
        ...
        $q = $this->db->get("jadwal_perawat");
        return $q->row();
    }

    function simpanjadwalperawat()
    {
        $jml = cal_days_in_month(CAL_GREGORIAN, date("m"), date("Y"));
        for ($i = 1; $i <= $jml; $i++) {
        $tanggal = $this->input->post("tahun") . "-" . $this->input->post("bulan") . "-" . $i;
        $this->db->where("bulan",$this->input->post('bulan'));
        $this->db->where("tahun",$this->input->post('tahun'));
        $this->db->where("id_perawat",$this->input->post('perawat'));
        $this->db->where("id_ruangan",$this->input->post('ruangan'));
        $m = $this->db->get("jadwal_perawat");
        if ($m->num_rows()>0) {
            $r = $m->row();
            $aksi = "edit";
        } else {
        ...
        $ket ["tgl".$i] =  array(
                "shift1"      => ($this->input->post('shift1_'. $i)!="" ? $this->input->post('shift1_'. $i) : ""),
                "shift2"      => ($this->input->post('shift2_'.$i)!="" ? $this->input->post('shift2_'.$i) : ""),
                "lepas"       => ($this->input->post('lepas_'.$i)!="" ? $this->input->post('lepas_'.$i) : ""),
                "libur"       => ($this->input->post('libur_'.$i)!="" ? $this->input->post('libur_'.$i) : ""),
                "sakit"       => ($this->input->post('sakit_'.$i)!="" ? $this->input->post('sakit_'.$i) : ""),
                "cuti"        => ($this->input->post('cuti_'.$i)!="" ? $this->input->post('cuti_'.$i) : ""),
                "dd"          => ($this->input->post('dd_'.$i)!="" ? $this->input->post('dd_'.$i) : ""),
                "dl"          => ($this->input->post('dl_'.$i)!="" ? $this->input->post('dl_'.$i) : ""),
                 );
        ...
        switch ($aksi) {
            ...
            $data = array(
                ...
                "tanggal"          => json_encode($ket),

            );

            $this->db->insert("jadwal_perawat", $data);
            $msg  = "success-Data Jadwal berhasil di simpan";
            return $msg;
            break;
            case 'edit':
            $data = array(
            ...
            $this->db->where("id_perawat",$this->input->post('perawat'));
            $this->db->where("id_ruangan",$this->input->post('ruangan'));
            $this->db->update("jadwal_perawat", $data);
            break;
        }
        $msg  = "success-Data Jadwal berhasil di simpan";
        return $msg;
    ...
    function hapusjadwalperawat($id,$ruangan)
    {
        ...
        $this->db->where("id_ruangan", $ruangan);
        $this->db->delete("jadwal_perawat");
        return "danger-Data Jadwal Perawat berhasil di hapus";
    }
    function getjadwalperawat_array($bulan,$tahun)
    {
        $this->db->order_by("id_perawat");
        $this->db->order_by("id_ruangan");
        $this->db->where("bulan", $bulan);
        ...
        $data = array();
        foreach ($q->result() as $key) {
          $data[$key->id_ruangan][$key->id_perawat] = json_decode($key->tanggal,true);
        }
        return $data;
    }
    function getbagian(){
      $this->db->group_by("kode_bagian");
      $q = $this->db->get("ruangan");
      return $q;
    }
    function getperawat_bagian($bagian=""){
        if ($bagian!="") {
          if ($bagian=="kontrole"){
            $this->db->where("p.kontrole",1);
          } else
          if ($bagian=="kepala"){
            $this->db->where("p.kepala_perawat",1);
          } else
          if ($bagian=="waka"){
            $this->db->where("p.wakil_kepala",1);
          } else
          {
            $this->db->where("p.bagian",$bagian);
          }
        }
        $this->db->order_by("kepala_perawat,wakil_kepala","desc");
        $q = $this->db->get("perawat p");
        return $q;
    }
    // function getkontrole(){
    //     $q = $this->db->get_where("perawat", ["kontrole" = 1]);
    //     return $q;
    // }
    function getkontrole($kontrole){
        $this->db->like('kontrole', $kontrole , 'both');
        $this->db->order_by('kontrole', 'ASC');
        $this->db->limit(10);
        return $this->db->get('perawat')->result();
    }
    function getcetakjadwal_perawat($bln)
    {
        $data = array();
        $this->db->select("jd.*, p.nama_perawat, r.nama_ruangan");
        $this->db->where("jd.bulan", $bln);
        $this->db->join("perawat p", "p.id_perawat = jd.id_perawat","inner");
        $this->db->join("ruangan r", "r.kode_bagian = jd.id_ruangan","inner");
        $this->db->order_by("r.kode_bagian,jd.id_perawat", "desc");
        $query = $this->db->get("jadwal_perawat jd");
        foreach ($query->result() as $row) {
            $data[$row->id_jadwal] = $row;
        }
    }
    function getbagian_array(){
      $this->db->group_by("kode_bagian");
      $q = $this->db->get("ruangan");
      $data = array();
      foreach ($q->result() as $key) {
        $data[$key->kode_bagian] = $key->nama_ruangan;
      }
      $data["kontrole"] = "KONTROLE";
      return $data;
    }
    function getcetak_perawat($id)
    {
        $this->db->select("pr.*,jtp.keterangan as jenistenaga,kw.nama as kawin,pe.pendidikan as nama_pendidikan");

        $this->db->join("pendidikan pe", "pe.idx=pr.id_pendidikan", "left");
        $this->db->join("jenis_tenaga_perawat jtp", "jtp.id_jtp=pr.id_jtp", "left");
        $this->db->join("kawin kw", "kw.kode=pr.status_kawin", "left");
        $this->db->order_by("pr.id_perawat", "desc");
        $query = $this->db->get_where("perawat pr", array("pr.id_perawat" => $id));
        return $query->row();
    }
    function getkarumkit(){
        $query = $this->db->get("setup_rs");
        return $query->row();
    }
    function getriwayatpangkat($id_perawat){
      $this->db->select("r.*,kp.keterangan as kenaikan");
      $this->db->join("kenaikan_pangkat kp","kp.id_kenaikan=r.id_kenaikan","left");
      $this->db->where("r.id_perawat", $id_perawat);
      $q = $this->db->get("riwayat_pangkat r");
      return $q;
    }
    function getriwayatjabatan($id_perawat){
      $this->db->select("r.*,j.keterangan as nama_jabatan");
      $this->db->join("jabatan j","j.kode_jabatan=r.jabatan","left");
      $this->db->where("r.id_perawat", $id_perawat);
      $q = $this->db->get("jabatan_perawat r");
      return $q;
    }
    function getriwayatpendidikan($id_perawat){
      $this->db->select("r.*,p.pendidikan as nama_pendidikan");
    ...
    function getriwayatkursus($id_perawat){
      $this->db->where("id_perawat", $id_perawat);
      $q = $this->db->get("kursus");
      return $q;
    }
    ...
    function getriwayatpenugasan($id_perawat){
      $this->db->select("r.*,p.name as provinsi,k.name as kota");
      $this->db->join("provinces p","p.id=r.id_provinsi","left");
      $this->db->join("regencies k","k.id=r.id_kota","left");
      $this->db->where("r.id_perawat", $id_perawat);
      $q = $this->db->get("penugasan r");
      return $q;
    }
    function getriwayatkeluarga($id_perawat){
      $this->db->where("id_perawat", $id_perawat);
      $q = $this->db->get("simpeg_keluarga");
      return $q;
    }
    function getpangkat_cpns($id_perawat){
      $this->db->select("r.*,kp.keterangan as kenaikan");
      $this->db->join("kenaikan_pangkat kp","kp.id_kenaikan=r.id_kenaikan","left");
      $this->db->where("r.id_kenaikan", 12);
      $this->db->where("r.id_perawat", $id_perawat);
      $q = $this->db->get("riwayat_pangkat r");
      return $q;
    }
    function getpangkat_awal($id_perawat){
      $this->db->select("r.*,kp.keterangan as kenaikan");
      $this->db->join("kenaikan_pangkat kp","kp.id_kenaikan=r.id_kenaikan","left");
      $this->db->like("status_pangkat","Pangkat Awal");
      $this->db->where("r.id_perawat", $id_perawat);
      $q = $this->db->get("riwayat_pangkat r");
      return $q;
    }
    function getpangkat_dahsboard(){
        $this->db->select("count(*) AS pangkat, p.keterangan as pangkat_dashboard");
        $this->db->group_by("p.keterangan");
        $this->db->order_by("p.id_pangkat");
        return $this->db->get("pangkat p");
    }
    function getlistpangkat_dashboard(){
        $now = date("Y-m-d");
