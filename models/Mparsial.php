<?php
class Mparsial extends CI_Model{
   function __construct()
	...
	function getparsial_detail($no_reg){
		$q = $this->db->get_where("transaksi_parsial",["no_reg"=>$no_reg]);
		return $q;
	}
	...
	function getparsial($no_reg){
		$this->db->join("tarif_ralan t","t.kode_tindakan=k.kode_tarif","inner");
		$q = $this->db->get_where("parsial k",["k.no_reg" => $no_reg]);
		return $q;
	}
	function getparsial_poli($no_reg){
		$this->db->join("tarif_ralan t","t.kode_tindakan=k.kode_tarif","inner");
		$q = $this->db->get_where("parsial_inap k",["k.no_reg" => $no_reg]);
		return $q;
	}
	...
	function addtindakan_lab(){
		$t = $this->db->get_where("tarif_lab",["id_tindakan" => $this->input->post("tindakan")]);
		if ($t->num_rows()>0){
			$data = $t->row();
			$tarif = $data->reguler;			
			$data = array(
	...
	function simpantransaksi(){
		...
		$q = $this->db->get_where("transaksi_parsial",["no_reg"=>$this->input->post("no_reg")]);
		if ($q->num_rows()>0){
			$this->db->where("no_reg",$this->input->post("no_reg"));
			$this->db->update('transaksi_parsial', $dat);
		} else {
			$this->db->insert('transaksi_parsial', $dat);
		}
		$this->db->where("no_reg",$this->input->post("no_reg"));
		$this->db->set("bayar","jumlah",FALSE);
		$this->db->set("id_transaksi",$id);
		$q = $this->db->update("parsial");
	}
	...
	function addtindakan_inap($jenis){
		...
		$this->db->insert("parsial_inap",$data);
		if ($jenis=="operasi"){
			$id++;
	...
	function gettindakan($no_reg){
		...
		if ($q->num_rows()>0) $poli = $q->row()->tujuan_poli; else $poli="";
	...
	function changedata_inap(){
		...
		$this->db->where("id",$this->input->post("id"));
		$this->db->update("parsial_inap",$data);
	}
	...
	function getkamar_array(){
		...
		$data = array();
		foreach ($q->result() as $key) {
			$data[$key->kode_ruangan."-".$key->kode_kelas] = $key->nama_ruangan." - ".$key->nama_kelas;
		}
		return $data;
	}
