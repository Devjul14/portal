...
class Dokter extends CI_Controller
{
    ...
    function hd($no_pasien = "", $no_reg = "")
    {
        $data["no_reg"]           = $no_reg;
        $data["no_pasien"]        = $no_pasien;
        $data["vmenu"]            = $this->session->userdata("controller") . "/vmenu";
        $data['menu']             = "dokter";
        $data["title"]            = "Form HD || RS CIREMAI";
        $data["title_header"]     = "Form HD ";
        $data["content"]          = "dokter/vhd";
        $data["breadcrumb"]       = "<li class='active'><strong>Form HD</strong></li>";
        $data["q1"]               = $this->Mdokter->getpasien_igdralan($no_reg);
        $data["q"]                = $this->Mdokter->gethd_detail($no_reg);
        $this->load->view('template', $data);
    }
    function simpanhd($aksi)
    {
        $message         = $this->Mdokter->simpanhd($aksi);
        $this->session->set_flashdata("message", $message);
        redirect("dokter/hd/" . $this->input->post("no_pasien") . "/" . $this->input->post("no_reg"));
    }
}
